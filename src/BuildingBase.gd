...
func place_building(grid_pos_: Vector2i, building_data_: BuildingData) -> void:
	...
	grid_pattern = GridPattern.new(building_data.grid_pattern.pattern, grid_pos_)


func _ready() -> void:
	self.modulate = modulate_unfinished

	grid_pattern = GridPattern.new(building_data.grid_pattern.pattern, Vector2i.ZERO)
