...
func distribute_jobs(dwarfs_looking_for_jobs: Array[Dwarf]) -> DwarfJobsAssignment:
	var job_set: Dictionary[AbstractJob, bool] = { }
	...
	for dwarf: Dwarf in dwarfs_looking_for_jobs:
		var scored_jobs: Array[ScoredJob] = Global.level.job_manager.aggregate_and_score_jobs_for_dwarf(dwarf)
	...
	var job_slots: Array[AbstractJob] = []
	for job: AbstractJob in job_set.keys():
...
func _hungarian_job_caluclation(cost_matrix: Array[Array], matrix_size: int) -> Array[int]:
	...
	for i: int in range(1, matrix_size + 1):
		p[0] = i
		var j0: int = 0
		var min_val: Array[float] = []
		var used: Array[bool] = []
		for k: int in range(matrix_size + 1):
			min_val.append(INF)
			used.append(false)
